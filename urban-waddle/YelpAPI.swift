...
func getRestaurants(near location: CLLocationCoordinate2D, completion: ((Result<YelpSearch>) -> Void)?) {
    var urlComponents = URLComponents()
    urlComponents.scheme = "https"
    urlComponents.host = "api.yelp.com"
    urlComponents.path = "/v3/businesses/search"
    let longitudeItem = URLQueryItem(name: "longitude", value: "\(location.longitude)")
    let latitudeItem = URLQueryItem(name: "latitude", value: "\(location.latitude)")
    let categoriesItem = URLQueryItem(name: "categories", value: "restaurants")
    urlComponents.queryItems = [longitudeItem, latitudeItem, categoriesItem]
